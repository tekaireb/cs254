import pyrtl
import pyrtl.rtllib.libutils as libutils

# Taken from PyRTL RTL Library
...
def _one_bit_add_no_concat(a, b, cin=0):
    # to make sure that an int cin doesn't break things
...
def wallace_reducer(wire_array_2, result_bitwidth, final_adder=kogge_stone):
    ...
    while not all(len(i) <= 2 for i in wire_array_2):
        ...
        for i, w_array in enumerate(wire_array_2):
            while len(w_array) >= 3:
...
def _sparse_adder(wire_array_2, adder):
    ...
    for single_w_index in range(len(wire_array_2)):
        ...
        if len(wire_array_2[single_w_index]) == 2:
            break
        result.append(wire_array_2[single_w_index][0])

    import six
    wires_to_zip = wire_array_2[single_w_index:]
    add_wires = tuple(six.moves.zip_longest(
        *wires_to_zip, fillvalue=pyrtl.Const(0)))
    adder_result = adder(pyrtl.concat_list(
        add_wires[0]), pyrtl.concat_list(add_wires[1]))
...
def fast_group_adder(wires_to_add, reducer=wallace_reducer, final_adder=kogge_stone):
    ...
    result_bitwidth = longest_wire_len + \
        int(math.ceil(math.log(len(wires_to_add), 2)))
